...
describe("HelpRequestsIndexPage tests", () => {
  ...
  test("renders three help requests correctly for regular user", async () => {
    ...
    const teamId = screen.getByText("team-1");
    expect(teamId).toBeInTheDocument();
    const tableOrBreakoutRoom = screen.getByText("16");
    expect(tableOrBreakoutRoom).toBeInTheDocument();
    const requestTime = screen.getByText("2025-05-05T01:01:05");
    expect(requestTime).toBeInTheDocument();
    const explanation = screen.getByText("the screen is broken");
